...
library(here)  #An alternative to setting a working directory manually
library(tidyverse)

NESdta <- read_csv(here("data", "anes_pilot_2016.csv"))
```
...
# viz to make sure the density of a feature (variable) looks the same between a training and testing dataset
tidy <- train_tidy %>% 
...
```{r}
library(patchwork)   #Combines a bunch of ggplot2 objects
...
See the JSS article introducing v2 of `mice` (though the package is currently on [v3.10.0](https://CRAN.R-project.org/package=mice)) for a ton of rich detail on the method, as well as many applications: <https://www.jstatsoft.org/article/view/v045i03>.
